...
if [ "$version" = "trusty" ] ; then
  ...
  wget  https://dl.google.com/dl/earth/client/current/google-earth-stable_current_i386.deb --no-check-certificate; 
...
if [ "$version" = "xenial" ] ; then
  ...
  wget --no-check-certificate http://ftp.fr.debian.org/debian/pool/main/l/lsb/lsb-core_4.1+Debian13+nmu1_amd64.deb
  wget --no-check-certificate http://ftp.fr.debian.org/debian/pool/main/l/lsb/lsb-security_4.1+Debian13+nmu1_amd64.deb 
  dpkg -i lsb*.deb ; dpkg -i google-earth*.deb ; apt install -fy ; rm -f lsb*.deb && rm -f google-earth*.deb
  
  # Celestia
  wget --no-check-certificate https://gitlab.com/simbd/Scripts_Ubuntu/-/blob/7925144bf30ed4c353b9676521d591dc35c97dde/Celestia_pour_Bionic.sh
...
if [ "$version" = "bionic" ] ; then
  ...
  wget --no-check-certificate https://dl.google.com/dl/earth/client/current/google-earth-pro-stable_current_amd64.deb ; dpkg -i google-earth-pro-stable_current_amd64.deb ; apt install -fy
...
if [ "$version" = "focal" ] ; then
  ...
  # Celestia
  #wget --no-check-certificate https://futureadressegit/Celestia_pour_focal.sh
...
if [ "$version" != "bionic" ] ; then  # Installation spécifique pour 14.04 ou 16.04
  ...
  wget --no-check-certificate http://www.openprinting.org/download/printdriver/debian/dists/lsb3.2/contrib/binary-amd64/openprinting-gutenprint_5.2.7-1lsb3.2_amd64.deb
...
if [ "$version" = "bionic" ] || [ "$version" = "focal" ] ; then
  # Openboard
  if [ "$version" = "bionic" ]; then
	  #wget --no-check-certificate https://gitlab.com/simbd/Scripts_Ubuntu/-/blob/7925144bf30ed4c353b9676521d591dc35c97dde/Openboard_1804.sh
  ...
  # GanttProject
  wget --no-check-certificate https://dl.ganttproject.biz/ganttproject-2.8.7/ganttproject_2.8.7-r2262-1_all.deb && dpkg -i ganttproject* ; apt install -fy ; rm ganttproject*
  # mBlock
  ...
  # OpenMeca
  wget --no-check-certificate http://d.a.d.a.pagesperso-orange.fr/openmeca-64b.deb && dpkg -i openmeca-64b.deb ; apt install -fy ; rm openmeca*
  # BlueGriffon
  wget --no-check-certificate http://bluegriffon.org/freshmeat/3.0.1/bluegriffon-3.0.1.Ubuntu16.04-x86_64.deb && dpkg -i bluegriffon*.deb ; apt install -fy ; rm bluegriffon*
